...
if(file.exists(file.path(params$workdir,'../raredisease_rnaseq/data/candidate_genes_LR.txt')) !=T) {
  ...
  #candidates
  candidates_SR = read.csv(file.path(params$workdir,'../raredisease_rnaseq/data/candidate_genes_3.txt'))
  ...
  #save the candidate genes with the long read data for the Shiny dashboard
  write.csv(candidates_LR,file.path(params$workdir,'../raredisease_rnaseq/data/candidate_genes_LR.txt'),row.names = F, quote = T)
} else candidates_LR = read.csv(file.path(params$workdir,'../raredisease_rnaseq/data/candidate_genes_LR.txt'), header = T)
